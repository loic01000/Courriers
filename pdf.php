...
{           
    ...
    header("X-Bar: Baz");
    header_remove(); 
  outPdf($courrier_id,$n);
  $n++;
  
}


function outPdf($courrier_id,$n)
{
    $envoi = (isset($_GET['envoi'])) ? $_GET['envoi'] : '';
    $db = new DB();
    $courrier = $db->sql("SELECT utilisateur_id, destinataire_id, vosref, nosref, objet, date_envoi, paragraphe1, paragraphe2, paragraphe3, paragraphe4 FROM courriers WHERE courriers.id= $courrier_id;");
    if($envoi == "ok")
    {
    $date = date('Y-m-d');
    $sql= "UPDATE `courriers` SET `status`= 'Envoye' , `date_envoi` = \"$date\" WHERE `id`=$courrier_id;";
    $db->sql($sql);
    }
    //print_r($courrier);
    ...
    //     // === Texte ================================
        ...
        $courrier = "courrier".$n."pdf";

        if($out=="I"){$pdf->Output('courrier$n.pdf',"I");}
        if($out=="D"){$pdf->Output('courrier.pdf',"D");}
